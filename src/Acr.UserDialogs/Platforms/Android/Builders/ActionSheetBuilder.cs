...
public class ActionSheetBuilder : IAlertDialogBuilder<ActionSheetConfig>
{
    public Dialog Build(Activity activity, ActionSheetConfig config)
    {
        ...
        if (config.ItemIcon != null || config.Options.Any(x => x.ItemIcon != null))
        {
            ...
            _ = dlg.SetAdapter((global::Android.Widget.IListAdapter)adapter, (s, a) => config.Options[a.Which].Action?.Invoke());
        }
        else
        {
            var array = config
            ...
            _ = dlg.SetItems(array, (s, args) => config.Options[args.Which].Action?.Invoke());
        }

        if (config.Destructive != null)
            _ = dlg.SetNegativeButton(config.Destructive.Text, (s, a) => config.Destructive.Action?.Invoke());

        if (config.Cancel != null)
            _ = dlg.SetNeutralButton(config.Cancel.Text, (s, a) => config.Cancel.Action?.Invoke());
    ...
    public Dialog Build(AppCompatActivity activity, ActionSheetConfig config)
    {
        ...
        if (config.ItemIcon != null || config.Options.Any(x => x.ItemIcon != null))
        {
            var adapter = new ActionSheetListAdapter(activity, Android.Resource.Layout.SelectDialogItem,
                Android.Resource.Id.Text1, config);
            _ = dlg.SetAdapter(adapter, (s, a) => config.Options[a.Which].Action?.Invoke());
        }
        else
        {
            var array = config
